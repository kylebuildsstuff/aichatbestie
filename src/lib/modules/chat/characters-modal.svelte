...
<div class="bg-white rounded-lg border-gray-200 divide-y">
  <div class="bg-white px-3 py-2">
    <h3 class="text-lg leading-6 font-medium text-gray-900">Characters</h3>
  </div>

  <div class="px-4 py-5 sm:p-6 overflow-auto">
    <ul
      class="grid grid-cols-2 gap-x-2 gap-y-4 sm:grid-cols-3 sm:gap-x-4 lg:grid-cols-5 xl:gap-x-8"
    >
      {#each voiceOptions as { voiceType: vType, voiceId: vId, voiceLabel }}
        ...
        <li class={`relative flex gap-0.5 items-center`}>
          <button
            class="rounded-full px-2.5 py-1.5 hover:cursor-pointer hover:bg-gray-100"
          >
            <SparklesIcon overrideClasses="w-5 h-5 text-gray-500" />
          </button>
          ...
          >
            <p class="mt-2 flex gap-1 items-center text-sm font-medium text-gray-900">
              ...
              <!-- Gender -->
              <span title="Male">
                <SparklesIcon overrideClasses="w-2.5 h-2.5 text-blue-500" />
              </span>
            </p>
            <p
  ...
  <div class="pt-5">
    <div class="flex flex-col items-end justify-end">
      ...
      <span class="flex justify-end mt-1 text-xs text-gray-500">
        Double-clicking the selection also applies the character.
      </span>
    </div>
  </div>
</div>
